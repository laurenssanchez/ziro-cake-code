...
class UsersController extends AppController {
	...
	public function login() {
		...
		if ($this->request->is('post')) {
			...
			$email=$this->validarDatosRaros($email, true);
			if ($email) {
				$alerta=true;
			}
			...
			$password=$this->validarDatosRaros($password, true);
			if ($password) {
				$alerta=true;
			}

			if ($alerta) {
				$this->Session->setFlash(__('Error en los datos enviados'), 'flash_error');
				return 5;
			}
	...
	public function send_code(){
		...
		if($time == 0 || $time < strtotime('now')){
			...
			$options = [
				"subject" 	=> "¡Ey, bienvenido al mundo de Zíro!",
				"to"   		=> AuthComponent::user("email"),
				"vars" 	    => [
					'code' => AuthComponent::user("code"),
				],
				"template"	=> "code_generated_user",
			];
	...
	public function remember_password(){
		...
		if ($this->request->is('post')) {
			...
			//limpiando input
			$email=filter_var($email, FILTER_SANITIZE_EMAIL);
			$email=$this->prepararParaSql($email);
			if (empty($email) || $email != $this->request->data['User']['email']) {
				$alerta=true;
			}

			if ($alerta) {
				$this->Session->setFlash(__('al consultar el correo electrónico'), 'flash_error');
				return 5;
			}
	...
	public function isStrongPassword($p)
	{
		return preg_match('/^((?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=\S+$)(?=.*[;:\.,!¡\?¿@#\$%\^&\-_+=\(\)\[\]\{\}])).{8,20}$/', $p)==1;
	}
}
