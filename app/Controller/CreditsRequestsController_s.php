...
class CreditsRequestsController extends AppController {
		...
		if(AuthComponent::user("role") == 5){
			...
			if(AuthComponent::user("role") == 3){
				...
				try {
					...
					$requestApply = [];//$this->CreditsRequest->find("all",["conditions"=>$conditionsApply,"recursive"=>$recursive, "order" => $order]);
				} catch (Exception $e) {
					$requestAdmin = [];
					$requestPendingStop = [];
					$requestApply = [];
				...
				if(!isset($this->request->query["ini"])){
					$fechaInicioReporte = date("Y-m-d",strtotime("-14 day"));
				}else{
				...
				if(!isset($this->request->query["tab"]) || (isset($this->request->query["tab"]) && !in_array($this->request->query["tab"], [1,2,3,4]) ) ){
					...
					try {
						...
						if (isset($this->request->query["ini"]) && isset($this->request->query["end"])) {
							$conditions["DATE(CreditsRequest.created) >=" ] = $this->request->query["ini"];
	...
	public function applyCredit(){
        ...
        $creditLineDetail = $this->CreditsRequest->query("SELECT * FROM credits_lines_details where credit_line_id = 5");
	...
	//	$this->Session->setFlash(__('Solicitud creada correctamente' . json_encode($creditLineDetail)), 'flash_success');
		...
		/*if ($valueCredit > 0 && $valueCredit<=100000) {

			$intRate 		= 1.88;
		    $intOther 		= 9.5;


		}else if($valueCredit >100001 && $valueCredit<=200000){

			$intRate 		= 1.88;
		    $intOther 		= 9.5;


	    }else if($valueCredit >200001 && $valueCredit<=300000){

			$intRate 		= 1.88;
			$intOther 		= 9.5;

		}else if($valueCredit >300001 && $valueCredit<=400000){

			$intRate 		= 1.0;
			$intOther 		= 4.0;

		}else if($valueCredit >400001 && $valueCredit<=500000){

			$intRate 		= 1.0;
			$intOther 		= 3.0;

		}else if($valueCredit >400001 && $valueCredit<=600000){

			$intRate 		= 1.0;
			$intOther 		= 2.5;

		}else if($valueCredit >600001 && $valueCredit<=700000){

			$intRate 		= 1.0;
			$intOther 		= 1.8;

		}else if($valueCredit >700001){
		...
		$credit = [
			"Credit" =>
				[
					...
					"interes_rate"	  => $intRate ,//$activeLine["CreditsLine"]["interest_rate"],
					"others_rate"	  => $intOther, // $activeLine["CreditsLine"]["others_rate"],
