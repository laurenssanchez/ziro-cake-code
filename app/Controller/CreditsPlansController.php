 ...
 */
	...
	public function admin(){
		...
		$juridico = strpos(strtolower($this->request->referer()), "juridico");
		$cobranza = strpos(strtolower($this->request->referer()), "cobranza");

		if (in_array(AuthComponent::user("role"), [9,11])) {
			$this->CreditsPlan->Credit->save($credito);
		}

		$this->CreditsPlan->setCobrosUnions();

		$type 		  = AuthComponent::user("role") == 11 || !empty($juridico) ? 1 : 0;
		...
		try {
			...
			$limit 		= 60;
			$endDay 	= 200;
			$customer 	= $this->CreditsPlan->Credit->Customer->field("identification",["id"=>$credito["Credit"]["customer_id"]]);
			$datosCuotas = $this->CreditsPlan->getQuotesCobranzas(null,$iniDay,$endDay,$customer,null,null,$start,$limit,true);
		} catch (Exception $e) {
			$datosCuotas = [];
		}

		if(AuthComponent::user("role") == 11 || !empty($juridico) ){
			$quoteInfo  = $this->CreditsPlan->getQuotesJuridico($quote);
		}else {
			$quoteInfo  = $this->CreditsPlan->getQuotesCobranzas($quote);
		}

		if (count($datosCuotas) == 1) {
			$datosCuotas["0"]["Credit"]["id"] = $credito["Credit"]["id"];
			$datosCuotas = [];
		}
