...
class ShopCommercesController extends AppController {
	...
	public function view($id = null) {
		...
		$users = $this->ShopCommerce->User->findAllByShopCommerceId($id);
	...
	public function users_disable($id){
		$action 					= 	'User';
		$id 						= 	$this->decrypt($id);
		$this->loadModel('User');
	...
	public function delete_data($id){
		$this->autoRender = false;
		$idShopCommerce = $this->decrypt($id);
		if (!$this->ShopCommerce->exists($idShopCommerce)) {
			throw new NotFoundException(__('Página no encontrada'));
		}
		$shopCommerce = $this->ShopCommerce->findById($idShopCommerce);

		if ($shopCommerce["ShopCommerce"]["state"] == 1) {
			$this->ShopCommerce->Shop->User->updateAll(["User.state" => 0 ], ["User.shop_commerce_id" =>$idShopCommerce]);
			$shopCommerce["ShopCommerce"]["state"] = 0;
		}else{
			$this->ShopCommerce->Shop->User->updateAll(["User.state" => 1 ], ["User.shop_commerce_id" =>$idShopCommerce]);
			$shopCommerce["ShopCommerce"]["state"] = 1;
		}

		$this->ShopCommerce->save($shopCommerce["ShopCommerce"]);
