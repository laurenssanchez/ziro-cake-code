...
class CreditsController extends AppController {
	...
	private function credOtorgadosExport($spreadsheet){
		...
		if (!empty($credits)) {
			...
			foreach ($credits as $key => $value) {
				...
				$spreadsheet->setActiveSheetIndex(0)->setCellValue('B'.$i, str_pad($value["Credit"]["credits_request_id"], 6, "0", STR_PAD_LEFT) );
		...
		if($query["tab"] == 1){
			...
			if(!empty($valuesQuotes)){
				foreach ($valuesQuotes as $key => $value) {
					$spreadsheet->setActiveSheetIndex(0)->setCellValue('A'.$i, str_pad($value["Credit"]["credits_request_id"], 6, "0", STR_PAD_LEFT) );
		...
		try {
			...
			if (!empty($receipts)) {
				foreach ($receipts as $key => $value) {
					...
					$spreadsheet->setActiveSheetIndex(0)->setCellValue('B'.$i, str_pad($this->Credit->field("credits_request_id",["id" => $value["CreditsPlan"]["credit_id"]]), 6, "0", STR_PAD_LEFT) );
					$spreadsheet->setActiveSheetIndex(0)->setCellValue('C'.$i, $customer["Customer"]["identification"] );
					$spreadsheet->setActiveSheetIndex(0)->setCellValue('D'.$i, $customer["Customer"]["name"]." ".$customer["Customer"]["last_name"] );
					$spreadsheet->setActiveSheetIndex(0)->setCellValue('E'.$i, $customer["CustomersPhone"]["0"]["phone_number"] );
					$spreadsheet->setActiveSheetIndex(0)->setCellValue('F'.$i, $customer["CustomersAddress"]["0"]["address"]. " ".$customer["CustomersAddress"]["0"]["address_city"]." ".$customer["CustomersAddress"]["0"]["address_street"] );
	...
	public function get_data_payment(){
		...
		$datos = [
			"name" => "Pago Crédito",
			"description" => "Pago Zíro, obligación #". str_pad($credit["Credit"]["credits_request_id"], 6, "0", STR_PAD_LEFT) ,
			"invoice" => date("YmdHis"),
			"currency" => "cop",
	...
	private function getSaldosByCredit($credits){
		...
		foreach ($credits as $key => $value) {
			...
			$totalByCredit[$this->encrypt($value["Credit"]["id"])] = [
				...
				],"fecha" => date("Y-m-d",strtotime($value["Credit"]["created"])),"numero" => str_pad($value["Credit"]["credits_request_id"], 6, "0", STR_PAD_LEFT),"commerce" => $commerceData["Shop"]["social_reason"] . " - " . $commerceData["ShopCommerce"]["name"]
			];
		}
		...
		foreach ($quotes as $key => $value) {
			if (is_null($first)) {
				...
				if($value["CreditsPlan"]["debt_value"] > 0 || $value["CreditsPlan"]["debt_honor"] > 0){
					...
					$ValorMora += $value["CreditsPlan"]["debt_value"]+$value["CreditsPlan"]["debt_honor"];
				}
			}
...
/*
		...
		if ($allCuotes == count($quotes) && $days <= 30  ) {
         ...
         //["Credit"]["interes_rate"] interes
		$interesesT =  ((($credit["Credit"]["value_pending"]* $credit["Credit"]["interes_rate"])/100)/30)*$days;


		$interesesOT =  ((($credit["Credit"]["value_pending"]* $credit["Credit"]["others_rate"])/100)/30)*$days ;

		//capital
	//	$CapitalN  = $credit["Credit"]["quota_value"] - $interesesOT  - $interesesT;

		$total		  	= $credit["Credit"]["value_pending"] + $interesesT + $interesesOT + $ValorMora;//$CapitalN + $interesesT + $interesesOT ; //+ $value["CreditsPlan"]["debt_value"]+$value["CreditsPlan"]["debt_honor"];
	...
	public function quotes_value(){
		...
		/*	foreach ($quotes as $key => $value) {
			...
			//$total =1;
		//	$total  = $this->getTotalFinal($creditId); Barranco
			foreach ($quotes as $key => $value) {
				$dataQuote 	= $this->Credit->CreditsPlan->getCuotesInformation($creditId,$value);
				$capital 	= $dataQuote["CreditsPlan"]["capital_value"]-$dataQuote["CreditsPlan"]["capital_payment"];
                $interes 	= $dataQuote["CreditsPlan"]["interest_value"]-$dataQuote["CreditsPlan"]["interest_payment"];
	...
	public function sendReceipt($quotes,$creditId,$dataWeb = null){
		...
		$options = [
			"subject" 	=> "Pago realizado al crédito: ".str_pad($credit["CreditsRequest"]["id"], 6, "0", STR_PAD_LEFT),
