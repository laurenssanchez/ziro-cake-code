...
class PagesController extends AppController {
	...
	public function beforeFilter() {
		parent::beforeFilter();
		$this->Auth->allow( 'home', 'register_step_one', 'register_step_two2', 'register_step_three', 'creditos','calculate','plan_payments','connect','fastpayment','politicas_uso_informacion','tyc','pagare','contrato','normal_request','commerce_payment','crediventas','generate_codes','validate_codes_crediventas');
	}
  ...
  public function crediventas() {
    ...
    if ($this->request->is("post")) {
      ...
      if(!$existsCommerce){
          return "El código de comercio no existe";
      }else{
          ...
          if(!empty($customer)){
            ...
            if(!empty($actualStudy)){
              return "Existe una solicitud en proceso en esté mismo comercio, no es posible tener dos al tiempo";
              die;
            }
          }
  ...
  public function register_step_four() {
    ...
    if($this->request->is("post")){
      ...
      if($this->request->data["Customer"]["code_email"] != $this->decrypt($this->request->data["Customer"]["code_email_verify"]) || $this->request->data["Customer"]["code_phone"] != $this->decrypt($this->request->data["Customer"]["code_phone_verify"]) ){
        $this->Session->setFlash(__('Error, los códigos no coinciden con los enviados'), 'flash_error');
      }else{
        ...
        $validTimeEmail = $this->CustomersCode->findByCodeAndCustomerIdAndTypeCodeAndState($this->request->data["Customer"]["code_email"],AuthComponent::user("customer_id"),1,0);

        $validTimePhone = $this->CustomersCode->findByCodeAndCustomerIdAndTypeCodeAndState($this->request->data["Customer"]["code_phone"],AuthComponent::user("customer_id"),2,0);

        if(empty($validTimeEmail) || empty($validTimePhone)){
          $this->Session->setFlash(__('Error, uno o los dos códigos expiraron su vigencia, revisa los códigos que fueron enviados nuevamente.'), 'flash_error');
        }else{
          $validTimeEmail["CustomersCode"]["state"] = 1;
          $validTimePhone["CustomersCode"]["state"] = 1;
