									...
									<?php if (!empty($creditsRequests)) : ?>
										<?php foreach ($creditsRequests as $creditsRequest) : ?>
											<tr>
												...
												<!-- Valor Solicitado -->
												<td>
													<?php echo number_format($creditsRequest['CreditsRequest']['request_value']); ?>&nbsp;
												</td>
												...
												<td>
													<?php if ($creditsRequest["CreditsRequest"]["shop_commerce_id"] == 0) : ?>
														Empresa: <?php echo $creditsRequest["Empresa"]["social_reason"] ?>
													<?php else : ?>
														<?php echo h($creditsRequest['ShopCommerce']['name'] . " - " . $creditsRequest['ShopCommerce']["Shop"]['social_reason']); ?>&nbsp;
													<?php endif ?>
												</td>
							...
							<table cellpadding="0" cellspacing="0" class="table table-striped table-hover">
								<thead class="">
									<tr>
										...
										<th><?php echo $this->Paginator->sort('value_disbursed', __('Retirado')); ?></th>
										<th><?php echo __('Acciones'); ?></th>
									</tr>
								</thead>
								<tbody>
									...
									if (!empty($creditsRequests)) : ?>
										<?php foreach ($creditsRequests as $creditsRequest) : ?>
											<tr>
												...
												<td>
													<?php if (!empty($creditsRequest["Credit"]["type"])) : ?>
														<?php
												...
												<td>
													$<?php echo !empty($creditsRequest["CreditsRequest"]["value_approve"]) ? number_format($creditsRequest["CreditsRequest"]["value_approve"]) : "" ?>
												</td>
												<!-- <td>
													<?php echo !empty($creditsRequest["CreditsRequest"]["number_approve"]) ? $creditsRequest["CreditsRequest"]["number_approve"] : "" ?>
												</td> -->
												<td>
													$<?php echo !empty($creditsRequest["CreditsRequest"]["value_disbursed"]) ? number_format($creditsRequest["CreditsRequest"]["value_disbursed"]) : "" ?>
												</td>
									...
									if (!empty($creditsRequests)) : ?>
										<?php foreach ($creditsRequests as $creditsRequest) : ?>
											<tr>
												...
												<?php endif ?>
												<td><?php echo h($creditsRequest['ShopCommerce']['name'] . " - " . $creditsRequest['ShopCommerce']["shop_name"]); ?>&nbsp;</td>
												<td>
			...
			<div class="x_content">
				<div class="row">
					<div class="col-md-12 steps-credits">
						<div class="row">
							<div class="col-xl-4 col-lg-6 col-md-6 credit-step1">
										...
										<div class="card sizecardtext">
											<div class="card-body">
												<h5 class="card-title">
													<strong class="text-right">
														...
														<?php if (!empty($value["CreditsRequest"]["empresa_id"])) : ?>
															<b class="badge-morado">EMPRESAS</b>
														<?php endif ?>
													</strong>
												...
												<div class="pr-2 mb-2 color">
													...
													<p class="card-text d-inline"><b>Celular:</b> <?php echo $value["Customer"]["celular"] ?></p>

													<p class="card-text d-inline">
														<?php if ($value["CreditsRequest"]["shop_commerce_id"] == 0) : ?>
															<b>Empresa</b>: <?php echo $value["Empresa"]["social_reason"] ?>
														<?php else : ?>
															<b>Proveedor:</b> <?php echo $value["ShopCommerce"]["shop_name"] ?> - <?php echo $value["ShopCommerce"]["name"] ?>
															<br><b>Frecuencia pago:</b>
															<?php
															if ($value["CreditsRequest"]["request_type"]  == 1)
																$tipoCredito = "Mensual";
															else if ($value["CreditsRequest"]["request_type"]  == 3)
																$tipoCredito = "45 días";
															else if ($value["CreditsRequest"]["request_type"]  == 4)
																$tipoCredito = "60 días";
															else
																$tipoCredito = "Quincenal";
							...
							<div class="col-xl-4 col-lg-6 col-md-6 credit-step2">
								<div class="tittle-column-credit">
							...
							<div class="col-xl-4 col-lg-6 col-md-6 credit-step3">
								<div class="tittle-column-credit">
								...
								<div class="credit-columns">
									...
									<?php foreach ($requestAdmin as $key => $value) : ?>
										...
										<div class="sizecardtext card <?php echo $value["CreditsRequest"]["state"] == 3 ? "bg-outline-green" : "bg-outline-red" ?> <?php if (!empty($value["CreditsRequest"]["empresa_id"])) : ?>morado_border<?php endif ?>">
											<div class="card-body">
												<h5 class="card-title">
													<strong class="text-right">
														<?php echo str_pad($value["CreditsRequest"]["id"], 6, "0", STR_PAD_LEFT); ?>
													...
													<?php endif ?>
													<br><?php echo $value["Customer"]["name"] ?> <?php echo $value["Customer"]["last_name"] ?> <br>
													<span class="asesor-gestor">
														<?php
														$pieces = explode(" ", $value['User']['name']);
														echo $pieces[0];
														?>
													</span>
												</h5>
												...
												<?php else : ?>
													<?php if ($value["CreditsRequest"]["state"] == 3) : ?>
														...
														<span class="money-approved morado text-center pull-right">
															<span>DECIDIDO EMPRESAS</span><b>
																Sugerido: $ <?php echo number_format($value["CreditsRequest"]["value_approve"]) ?> </b>
														</span>
													<?php else : ?>
														<div class="bg-morado p-1 pull-right text-center text-white">
															DECIDIDO EMPRESAS
														</div>
													<?php endif ?>
												<?php endif ?>
				...
				<div class="x_content form100">
						...
						<div class="col-md-3">
							<div class="form-group top_search">
								...
								<div class="input-group">
									<input name="ccCustomer" placeholder="Buscar por cédula" class="form-control" value="<?php echo isset($ccCustomer) ? $ccCustomer : "" ?>" type="text" id="ccCustomer">
									<span class="input-group-btn">
					...
					<div class="table-responsive mt-2">
						<table cellpadding="0" cellspacing="0" class="table table-striped table-hover">
							...
							<tbody>
								<?php if (!empty($creditsRequests)) : ?>
									<?php foreach ($creditsRequests as $key => $value) : ?>
										<tr>
											<td>
												<?php
											...
											<td>
												$<?php echo number_format($value["CreditsRequest"]["value_approve"]) ?>
											</td>
											<td>
											...
											<td>
												<a href="#" class="card-link btn btn-outline-secondary viewCustomerRequest" data-customer="<?php echo $this->Utilidades->encrypt($value["Customer"]["id"]) ?>" data-request="<?php echo $this->Utilidades->encrypt($value["CreditsRequest"]["id"]) ?>">
													<i class="fa fa-eye" data-toggle="tooltip" data-placement="top" title="" data-original-title="Ver solicitud"></i>
												</a>
...
<?php elseif (AuthComponent::user("role") == "4" || AuthComponent::user("role") == "6" || AuthComponent::user("role") == "7") : ?>
	<ul class="nav nav-tabs controltabs">
		...
		<li class="nav-item">
			<a class="nav-link <?php echo $this->request->query["tab"] == 4 ? "active" : "" ?>" href="<?php echo $this->Html->url(array("controller" => "credits_requests", "action_index", "?" => ["tab" => 4])) ?>">Solicitudes negadas</a>
		</li>
		<li class="nav-item">
								...
								<?php if (!empty($creditsRequestsCommerce)) : ?>
									<?php foreach ($creditsRequestsCommerce as $key => $value) : ?>
										<tr>
											...
											<td>
												<?php echo date("d-m-Y h:i:s A", strtotime($value['CreditsRequest']['created'])); ?>
											...
											<td>
												...
												<?php elseif (in_array($this->request->query["tab"], [2])) : ?>
													...
													<a rel="tooltip" href="" class="btn btn-secondary deatil_payment"
														data-total="<?php echo $value['CreditsRequest']['value_approve'] ?>"
														data-number="<?php echo $value['CreditsRequest']['number_approve'] ?>"
														data-frecuency="<?php echo $value["CreditsRequest"]["request_type"] ?>">
														<i class="fa fa-usd"
															data-toggle="tooltip"
															data-placement="top"
															title=""
													...
													<!-- solicitar retiro -->
													<a data-toggle="modal"
														data-target="#credit_applied"
														class="card-link btn btn-primary btn-xs text-white applyCredit"
														data-type="<?php echo empty($value["Customer"]["email"]) ? 0 : 1; ?>"
														data-frecuency="<?php echo $value["CreditsRequest"]["request_type"] ?>"
														data-value="<?php echo $value["CreditsRequest"]["value_approve"] ?>"
														data-numberq="<?php echo $value["CreditsRequest"]["number_approve"] ?>"
														data-return="<?php echo $value["CreditsRequest"]["returned"] ?>"
														data-id="<?php echo $this->Utilidades->encrypt($value["CreditsRequest"]["id"]); ?>">
...
<?php if (in_array(AuthComponent::user("role"), [1, 2, 3, 4, 6, 7, 15])) : ?>
	<script>
