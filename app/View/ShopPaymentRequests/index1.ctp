		...
		<div class="col-md-12">
			<div class="paymentsblock">
				...
				<div class="row">
					...
					<div class="col-md-1">
						<button class="btn btn-success mt3" type="submit">
							<?php echo __('Buscar'); ?>
						</button>
					...
					<div class="col-md-12 text-center pt-3">
						<?php if (!empty($shopPaymentRequests)) : ?>
							<?php if (isset($shopPaymentRequests[1])) : ?>
								<b>Se encontrarón <?php echo count($shopPaymentRequests[1]) ?> resultado(s) para PAGO1</b>
							<?php endif ?>
							<?php if (isset($shopPaymentRequests[1]) && isset($shopPaymentRequests[2])) : ?>
								<span class="mx-2"> / </span>
							<?php endif ?>
							<?php if (isset($shopPaymentRequests[2])) : ?>
								<b>Se encontrarón <?php echo count($shopPaymentRequests[2]) ?> resultado(s) para PAGO2</b>
							<?php endif ?>
						<?php else : ?>
								...
								<?php if (!empty($shopPaymentRequests)) : ?>
									<?php foreach ($shopPaymentRequests as $shopPaymentRequest) : ?>
										<tr>
											...
											<td><?php echo !is_null($shopPaymentRequest['ShopPaymentRequest']['final_date']) ? date("d-m-Y", strtotime(h($shopPaymentRequest['ShopPaymentRequest']['final_date']))) : ""; ?>&nbsp;</td>
											<td><?php echo date("d-m-Y", strtotime(h($shopPaymentRequest['ShopPaymentRequest']['request_date']))); ?>&nbsp;</td>
											<td>
									...
									<?php if (!empty($shopPaymentRequests[1])) : ?>
										...
										<?php foreach ($shopPaymentRequests[1] as $shopPaymentRequest) : ?>
											<tr>
												...
												<td>
													<?php echo $shopPaymentRequest['Shop']['account_bank']; ?>
												...
												<td>
													<?php echo $shopPaymentRequest['User']['name'] . " - " . $shopPaymentRequest["Shop"]["code"]; ?>
												</td>
												<td>
													...
													<tbody>
														<?php if (!empty($requests)) : ?>
															<?php foreach ($requests as $request) : ?>
																<tr>
																	<td>$<?php echo number_format($request['ShopPaymentRequest']['request_value'], 2); ?>&nbsp;</td>
																	<td><?php echo number_format($request['ShopPaymentRequest']['final_value'], 2); ?>&nbsp;</td>
																	<td><?php echo is_null($request['ShopPaymentRequest']['final_date']) ? "" : date("d-m-Y ", strtotime(h($request['ShopPaymentRequest']['final_date']))); ?>&nbsp;</td>
																	<td><?php echo date("d-m-Y", strtotime(h($request['ShopPaymentRequest']['request_date']))); ?>&nbsp;</td>
																	<td>
																		<?php echo $request['Shop']['social_reason']; ?>
																	</td>
																	<td>
																	...
																	<td>
																		<?php echo $request['User']['name'] . " - " . $request["Shop"]["code"]; ?>
																	</td>
																	<td>
...
<?php
	// echo $this->Html->script("payments/requests.js?".rand(),array('block' => 'AppScript'));
?>
