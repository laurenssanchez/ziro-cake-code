<div class="page-title">
  <div class="title_left">
    <h3><?php echo __('Notes'); ?>    	<a class="btn btn-info pull-right" href="<?php echo $this->Html->url(array('controller'=>$this->request->controller,'action'=>'add')); ?>">
		...
		<div class="x_panel">
			<div class="x_content">
				<div class="table-responsive">
					<?php if(empty($notes) && !empty($this->request->query['q'])) : ?>
...
<?php 					 foreach ($notes as $note): ?>
							<tr>
										<td>
									<?php echo $this->Html->link($note['CreditsPlan']['id'], array('controller' => 'credits_plans', 'action' => 'view', $note['CreditsPlan']['id'])); ?>
								</td>
								<td><?php echo h($note['Note']['note']); ?>&nbsp;</td>
								<td>
									<?php echo $this->Html->link($note['User']['name'], array('controller' => 'users', 'action' => 'view', $note['User']['id'])); ?>
								</td>
								<td> <?php echo $note['Note']['state'] == 1 ? __('Activo') : __('Inactivo') ;?> </td>									<td class="td-actions">
									    <a rel="tooltip" href="<?php echo $this->Html->url(array('action' => 'view',$this->Utilidades->encrypt($note['Note']['id']))); ?>" title="<?php echo __('Ver'); ?>" class="btn btn-info  btn-xs">
									        <i class="fa fa-eye"></i>
									    </a>
									    <a rel="tooltip" href="<?php echo $this->Html->url(array('action' => 'edit',$this->Utilidades->encrypt($note['Note']['id']))); ?>" title="<?php echo __('Editar'); ?>" class="btn btn-success btn-xs">
									        <i class="fa fa-edit"></i>
									    </a>
									    <a rel="tooltip" href="<?php echo $this->Html->url(array('action' => 'delete',$this->Utilidades->encrypt($note['Note']['id']))); ?>" title="<?php echo $note['Note']['state'] == 1 ? __('Deshabilitar') : __('Habilitar'); ?>" class="btn btn-danger btn-xs changeState">
									    	<?php if($note['Note']['state'] == 1): ?>									        <i class="fa fa-times-circle-o"></i>
