		...
		<div class="datacliente">
			...
			<p class="upper"><b>ALMACÉN:</b>
				<?php if (!empty($dataPayment["ShopCommerce"])): ?>
					<?php echo $shop["Shop"]["social_reason"] ?> -
					<?php echo $dataPayment["ShopCommerce"]["name"] ?>
				<?php else: ?>
					PAGO WEB
				<?php endif ?>
			</p>
			...
			<?php if (!empty($receipt["Payment"])): ?>
				...
				<?php foreach ($totalByCuotes as $key => $value): ?>
					...
					<?php else: ?>
						<?php if ($key == $last): ?>
							<!-- <p class="upper">Saldo Crédito: <b>$<?php //echo number_format($totalCredit < 1000 ? 0 : $totalCredit); ?></b></p> -->
						<?php else: ?>
			...
			<p id="statushere" class="upper <?php echo number_format($receipt["Receipt"]["saldo"] < 1 ? 0 : $receipt["Receipt"]["saldo"]) ?>">ESTADO CRÉDITO: <b>
