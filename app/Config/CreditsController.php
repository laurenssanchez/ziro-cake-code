...
class CreditsController extends AppController
{
    ...
    public function beforeFilter()
    {
      ...
      //  $this->Auth->allow('search_user', 'set_debts', 'get_credit_customer', 'get_data_payment', "payment_web", "payment_web_response");
		$this->Auth->allow('search_user', 'set_debts', 'get_credit_customer', 'get_data_payment', "payment_web", "payment_web_response","payment_crediventas","get_payment_data","plan_payemts_pdf","filter_crediventas");
	}
	...
	public function get_payment_data(){
        ...
        $customerData = $this->Customer->findByIdentification($identification);
    ...
    private function datacredito($data, $conditions)
    {
        ...
        if (!empty($credits)) {
            ...
            foreach ($credits as $key => $value) {
                ...
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('S' . $i, date("Ymd", strtotime($value["Credit"]["last_payment_date"])));
    ...
    public function payment_detail($creditId)
    {
	   ...
	   $fecmin = $this->Credit->query("SELECT MIN(payments.CREATED) as fechamin  from payments where payments.receipt_id  IS NULL");
        ...
        $totalCap = $this->Credit->CreditsPlan->find("first", ["conditions" => ["credit_id" => $value["CreditsPlan"]["credit_id"]], "fields" => ["SUM(capital_value) as total"]]);
        ...
        if (!empty($totalCap)) {
            ...
            if ($creditInfo["CreditsRequest"]["value_disbursed"] > $totalCap) {
                ...
                $plan_id = $this->Credit->CreditsPlan->field("CreditsPlan.id", ["credit_id" => $value["CreditsPlan"]["credit_id"]], ["id" => "DESC"]);
                //"order" =>["id"=>"DESC"]
                $valorUltQ = $this->Credit->CreditsPlan->field("CreditsPlan.capital_value", ["credit_id" => $value["CreditsPlan"]["credit_id"]], ["id" => "DESC"]);
        ...
        for ($i = 0; $i < sizeof($quotes); $i++) {
            ...
            $pay = $this->Credit->query("select sum(value) as PaymentA from payments where credits_plan_id= ' " . $quotes[$i]["CreditsPlan"]["id"] . " ' ");
            $quotes[$i]["CreditsPlan"] += ["TotalAbo" => $pay[0][0]["PaymentA"]];
        }

        $this->set(compact("creditRequest", "layout", "creditInfo", "quotes", "totalCredit","fecmin"));
    }
    ...
    public function payment_quotes($dataWeb = null)
    {
        ...
        if ($type == "2") {
			...
			if ($valorPago == $totalApagar) { // si valor digitado es igual al total de la deuda
                $quotes = $this->Credit->CreditsPlan->getCuotesInformation2($creditId, null, 0);
                $this->paymentTotalCredit($quotes, $creditId, $dataWeb); //llamo a la funcion pagar todo
            } else {
                $quotes = $this->Credit->CreditsPlan->getCuotesInformation2($creditId, null, 0); // obtengo su estado actual por couta
                foreach ($quotes as $key => $value) {
            ...
            if (($totalCuotesNoPayment != count($quotes) && $totalCuotesNoPayment != 0) || ($totalCuotesNoPayment == 1 && count($quotes) == 1)) {
				...
				$quotes = $this->Credit->CreditsPlan->getCuotesInformation2($creditId, $quotes, 0);
    ...
    private function payment_cuotes_normal($valorPago, $value, $capital = null, $dataWeb = null)
    {
        ...
        if ($value["CreditsPlan"]["debt_value"] > 0 && $valorPago > 0) {
            ...
            $paymentDebt = [
                "Payment" => [
                    ...
                    "value" => $valorPago > $debt_value ? $debt_value : $valorPago,
        ...
        if ($value["CreditsPlan"]["debt_honor"] > 0 && $valorPago > 0) {
            ...
            $paymentHonor = [
                "Payment" => [
                    "credits_plan_id" => $value["CreditsPlan"]["id"],
                    "value" => $valorPago > $debt_honor ? $debt_honor : $valorPago,
                    "user_id" => AuthComponent::user("id"),
                    "shop_commerce_id" => AuthComponent::user("role") == 11 ? $credit["CreditsRequest"]["shop_commerce_id"] : AuthComponent::user("shop_commerce_id"),
                    "type" => 5,
                    "web" => $dataWeb,
    ...
    private function paymentTotalCredit($cuotes, $creditId, $dataWeb = null, $totalApagar = 0)
    {
        if ($totalApagar == 0) {
            $num = 0;
