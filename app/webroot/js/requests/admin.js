...
$("body").on("click", "#descargaIMG", function (event) {
	event.preventDefault();
	$("#preloader").show();
	var id = $(this).data("id");
	$.post(
		root + "customers/download_img",
		{ id },
		function (data, textStatus, xhr) {
			$("#preloader").hide();
		}
...
function callModalRequst(request) {
	$("#commentsBody").html();
	$.post(
		root + "credits_requests_comments/view_comment",
		{ request },
		function (data, textStatus, xhr) {
...
$("body").on(
	"submit",
	"#CreditsRequestsCommentViewCommentForm",
	function (event) {
		event.preventDefault();
		$.post(
			root + "credits_requests_comments/save_comment",
			$(this).serialize(),
			function (data, textStatus, xhr) {
				if ($.trim(data) === "1") {
					location.reload();
	...
	$.post(
		...
		function (data, textStatus, xhr) {
			...
			setTimeout(function () {
				...
				if ($("body").find("#CreditsRequestsCommentEmpresasForm").length) {
					$("body").find("#CreditsRequestsCommentEmpresasForm").parsley();
				}
			}, 500);
		}
	);
...
$("body").on("keyup", "#CreditsRequestNumberApprove", function (event) {
	setTimeout(function () {
		...
		var frecuency = $("#CreditsRequestTypeRequest").val();
		if (
			($("#CreditsRequestNumberApprove").val() == "" ||
				parseInt($("#CreditsRequestNumberApprove").val()) > numReferenceOne) &&
			frecuency == "1"
		) {
		...
		if (
			($("#CreditsRequestNumberApprove").val() == "" ||
				parseInt($("#CreditsRequestNumberApprove").val()) > numReferenceTwo) &&
			frecuency == "2"
		) {
...
$("body").on("submit", "#approveRequestForm", function (event) {
	...
	$("#preloader").show();
	$.post(
		root + controller + "/approve/" + $("#Rdnb").val(),
		$(this).serialize(),
		function (data, status, error) {
			location.reload();
		}
...
$("body").on(
	"change",
	"#valueCredit,#valueNumberQ,#frecuency",
	function (event) {
		setTimeout(function () {
			getValueCuota();
		}, 1000);
...
function getValueCuota() {
	$.post(
		root + "pages/calculate",
		{
			couteValue: $("#valueNumberQ").val(),
			priceValue: $("#valueCredit").val(),
			frecuency: $("#frecuency").val(),
		},
		function (data, textStatus, xhr) {
			data = $.trim(data);
			$(".textCuote").html("Valor cuota: $" + data);
			$(".textCuote").attr("data-valor", data);
		}
	);
}
...
$(".viewReciboPago").click(function (event) {
	...
	var id = $(this).data("request");
	$.post(
		root + controller + "/receipt_payment/" + id,
		{},
		function (data, textStatus, xhr) {
...
$("body").on("click", ".detailCredit", function (event) {
	...
	$("#preloader").show();
	$.post(
		root + controller + "/credit_detail/" + id,
		{},
		function (data, textStatus, xhr) {
...
$("body").on("click", "#searchCentral", function (event) {
	...
	$.post(
		root + controller + "/consult_central/",
		{ request: request },
		function (data, textStatus, xhr) {
	...
	Swal.fire({
		...
		inputValidator: (reason) => {
			...
			if (!reason) {
				...
				$("#preloader").show();
				$.post(
					root + "credits_requests_comments/return_request",
					{ id, reason },
					function (data, textStatus, xhr) {
						location.reload();
					}
...
$("body").on("click", ".returnRequestData", async function (event) {
	...
	$.post(
		root + "credits_requests_comments/return_to_approved",
		{ id, reason: formValues[0] },
		function (data, textStatus, xhr) {
			location.reload();
		}
	);
...
$("#btnCustomerSearch").click(function (event) {
	...
	if ($.trim(cc) == "") {
		...
		$.post(
			root + "/credits_requests/search_customer",
			{ cc },
			function (data, textStatus, xhr) {
				$("#dataCustomerDataPayment").html(data);
			}
...
$("body").on("submit", "#DocumentGetDataCustomersForm", function (event) {
	event.preventDefault();
	var FORMULARIO_VALIDO = false;
	preloader(true);

	var form = $("#DocumentGetDataCustomersForm")[0];
	var formData = new FormData(form);

	$.ajax({
		type: "POST",
		url: root + "customers/add_document",
		data: formData,
		processData: false,
		contentType: false,
		cache: false,
		success: function (response) {
			...
			setTimeout(function () {
				preloader(false);
				$("body").find("#documents-tab").trigger("click");
			}, 1000);
		},
		error: function (e) {
			console.log(e);
...
$("#valueCredit").change(function (event) {
	if ($(this).val() > 750000000000000000000000000) {
		$(this).val(750000000000000000000000000);
	}
});
...
$("#enviarCodigosCliente,#reenviarCodigos").click(function (event) {
	...
	event.preventDefault();
	setTimeout(() => {
		var cadena = $("#id_request").val();
		request=cadena.replace(/(\r\n|\n|\r)/gm, "");
		var valorCredito = $("#valueCreditApplied #valueCredit").val();
		var cuotasCredito = $("#valueCreditApplied #valueNumberQ").val();
		var cuotaCredito = $(".textCuote").attr("data-valor");
		$.post(root + controller + "/sendCodesCredit",{valorCredito, cuotaCredito, cuotasCredito,request},
			function (data, textStatus, xhr) {
				data = $.parseJSON(data);
				$("#preloader").hide();
...
$("#validarCodigos").click(function (event) {
	...
	if (codePhone == "") {
		...
		$.post(
			root + controller + "/validateCode",{ request, codeMailRequest, codePhoneRequest, codeMail, codePhone },
			function (data, textStatus, xhr) {
				if ($.trim(data) == "1") {
					showMessage(
...
$("body").on("click", ".applyCreditNew", function (event) {
	...
	Swal.fire({
		...
		if (result.value) {
			$.post(
				root + controller + "/create_request_approved",
				{ value, customer, commerce },
				function (data, textStatus, xhr) {
					if (data != "0") {
						var numberq = 4;
						var id = data;
...
function calculateFinal(){
	...
	console.log('calculateFinal' + frecuency)
