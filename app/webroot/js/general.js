...
window.Parsley.addValidator(
	...
	32
).addMessage("es", "fileall", "La extención del archivo debe ser PDF,PNG,JPG");
...
$("body").on("click", ".deleteDoc", function (event) {
	event.preventDefault();
	let url = $(this).attr("href");
	console.log(BTB_SEARCH);

	Swal.fire({
		title: "Confirmar",
		text: "¿Está seguro de borrar el documento?",
		type: "warning",
		showCancelButton: true,
		confirmButtonColor: "#3085d6",
		cancelButtonColor: "#d33",
		...
		if (result.value) {
			if (typeof BTB_SEARCH != "undefined" && BTB_SEARCH != null) {
				preloader(true);
		...
		function (data, textStatus, xhr) {
			...
			if (data == "6") {
				...
				if (data == "0" || data == "1" || data == "5") {
					...
					if (data == "7") {
						location.href = varsJs.APP_URL + "pages/dashboardcliente";
					} else if (data == "2") {
