...
function addColorData(){
	document.querySelectorAll(".__range").forEach(function(el) {
	  ...
	  var valPercent = (el.valueAsNumber  - parseInt(el.min)) /
	                      (parseInt(el.max) - parseInt(el.min));
	    var style = 'background-image: -webkit-gradient(linear, 0% 0%, 100% 0%, color-stop('+ valPercent+', #bbffbf), color-stop('+ valPercent+', #f3f3f3));';
	    el.style = style;
	  };
	  el.oninput();
	});
...
function calculate(){
	addColorData()
	const frecuency  = $("#frecuency").val();
	console.log('calculate' + frecuency)
	...
	$.post(varsJs.APP_URL+"general/calculate", {priceValue,couteValue,frecuency,idsLine}, function(data, textStatus, xhr) {
		$("#valueCalculated").html(data);
	});
}
