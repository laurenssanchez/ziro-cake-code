...
if (statedebt == "$0"){
	$(".valuedebt").html("<span class='text-success'><b>$0 CRÉDITO PAGADO</b></span>");
}
...
window.Parsley.addValidator('imageextension', function (value, requirement) {
...
.addMessage('es', 'imageextension', 'La extención del archivo debe ser PNG o JPG');
...
var diasantes = day - 5;
if (diasantes == 0){ var diasantes = diasantes + 1;}
...
$("body").on("submit","#UserLoginForm",function(event) {
	...
	$.post($(this).attr("action"), $(this).serialize(), function(data, textStatus, xhr) {
		...
		data = $.trim(data);
		if(data == "6"){
			showMessage("Actualmente tu usuario se encuentra deshabilitado ya que, te encuentras en cobro jurídico. Comunicate con soporte",true);
		}else if(data == "4"){
			...
			if(data == "0" || data == "1" || data == "5"){
				...
				if(data == "2"){
					...
					location.href = varsJs.APP_URL+"credits_requests";
				}else{
...
$("body").on('click', '#validateBtn', function(event) {
	...
	if($.trim(codigo) == ""){
		...
		$.post(varsJs.APP_URL+"users/validate_code", {codigo}, function(data, textStatus, xhr) {
			$("#preloader").hide();
			if($.trim(data) == "0"){
...
$("body").on('click', '#reenvioBtn', function(event) {
	event.preventDefault();
	$("#preloader").show();
	$.post(varsJs.APP_URL+"users/send_code", {}, function(data, textStatus, xhr) {
		$("#preloader").hide();
