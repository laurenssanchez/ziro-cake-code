...
$("body").on('click', '.adminQuote', function(event) {
	...
	var params = $(this).data("parametro");

	preloader();
	$.post(root+"credits_plans/admin", {quote,credit,tab,params}, function(data, textStatus, xhr) {
		$("#cuerpoCob").html(data);
...
$("body").on('submit', '#formPaymentTotal', function(event) {
	event.preventDefault();
	preloader(true);
	var creditId = $("#creditIDData").val();
	var totalPago = $("#otherValueNum").val();
