...
function funcionSiguiente() {
	...
	if (positionStep === 4) {
		paso = false;
		if ($("#priceValue").val() < 200000 || $("#priceValue").val() > 20000000) {
			// showMessage(`Debe solicitar un cupo entre 200.000 a 10.000.000`, true);
			Swal.fire({
				icon: "error",
				title: "Error",
				text: "Debe solicitar un cupo entre $200.000 a $20.000.000",
			});
			paso = false;
		} else {
			paso = true;
		...
		if (
			...
			// se valida que si el codigo del comercio es de superbeauty se debe digitar obligatoriamente el user_id
			if (
				$("#CustomerUserIdCommerce").val() === "" &&
				$("#CustomerCode").val() === "39277219"
			) {
				Swal.fire({
					icon: "error",
					title: "Error",
					text: "El campo user_id es obligatorio lo puedes consolar en tu plataforma de superbeauty o contacta un asesor",
				});
			} else {
				positionStep = 5;
				$("#step4").hide();
...
function registroMetamapVerificacion() {
	const settings = {
		...
		processData: false,
		data:
			'{"metadata":{"plataforma":"ziro cake php","urlAliado":"' +
...
function registroMetamapObtenerDatosDocumento() {
	...
	$.ajax(settings).done(function (responsehookVerification) {
		...
		var dataHookVerification = responsehookVerification.documents[0];
		// Valida si ya tiene el elemento field que contiene la informacion del documento para poder continuar
		let tieneClave = dataHookVerification.steps[10].hasOwnProperty("data");

		if (tieneClave) {
			// valida que el score del usuario sea mayor a 20 el score es la veracidad del documento y la foto
			if (dataHookVerification.steps[8].data.score > 20) {
				// si ya tiene cargada la informacion en el objeto de la posicion 11 continue sino reintente
				if (
					dataHookVerification.steps[10].data.firstName.value !== null &&
					dataHookVerification.steps[10].data.surname.value !== null &&
					dataHookVerification.steps[10].data.documentNumber.value !== null
				) {
					$("#CustomerName").val(
						dataHookVerification.steps[10].data.firstName.value
					);
					$("#CustomerLastName").val(
						dataHookVerification.steps[10].data.surname.value
					);
					limpiarNumeroDocumento =
						dataHookVerification.steps[10].data.documentNumber.value.replace(
							/\./g,
							""
						);
					$("#CustomerIdentification").val(limpiarNumeroDocumento);

					//desactiva el loading en pantalla
					$("#preloader").hide();
					...
					if (
						...
						formData.append("score", dataHookVerification.steps[8].data.score);
